...
# The following function returns a dictionary, in which each item has a UniProt
# ID as its key, and the corresponding set of GO terms as its value. The code:
# s = go_dict["Q9BS40"]
# would store the set of GO terms associated to protein Q9BS40 in 's'.
#
# If a cachefile is specified as the script's [optional] 5th input argument, the
# function checks to see if this file already exists. If it does, the GO terms
# are read from the specified file. If it does not, the GO terms are retrieved
# through EBI's QuickGO webservice.
#
